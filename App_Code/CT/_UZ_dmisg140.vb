...
Namespace SIS.DMISG
  Partial Public Class dmisg140
    ...
    Public Shared Function dmisg140GetByDocumentID(ByVal t_docn As String) As SIS.DMISG.dmisg140
      ...
      Return Results
    End Function
    Public Shared Function UpdatePOAD(ByVal t_docn As String, ByVal trdt As String) As SIS.DMISG.dmisg140
      Dim Results As SIS.DMISG.dmisg140 = Nothing
      Dim Comp As String = "200" ' It should be byval parameter latter
      If Comp Is Nothing Then Comp = "200"
      If Comp = "" Then Comp = "200"
      Using Con As SqlConnection = New SqlConnection(SIS.SYS.SQLDatabase.DBCommon.GetBaaNConnectionString())
        Using Cmd As SqlCommand = Con.CreateCommand()
          Cmd.CommandType = CommandType.Text
          Cmd.CommandText = "update tdmisg140" & Comp & " set t_poad = convert(datetime,'" & trdt & "',103) where t_docn='" & t_docn & "' and t_poad < convert(datetime,'31/12/2000',103)"
          Con.Open()
          Cmd.ExecuteNonQuery()
        End Using
      End Using
    ...
    Public Shared Function UpdatePOIS(ByVal t_docn As String, ByVal trdt As String) As SIS.DMISG.dmisg140
      Dim Results As SIS.DMISG.dmisg140 = Nothing
      Dim Comp As String = "200" ' It should be byval parameter latter
      If Comp Is Nothing Then Comp = "200"
      If Comp = "" Then Comp = "200"
      Using Con As SqlConnection = New SqlConnection(SIS.SYS.SQLDatabase.DBCommon.GetBaaNConnectionString())
        Using Cmd As SqlCommand = Con.CreateCommand()
          Cmd.CommandType = CommandType.Text
          Cmd.CommandText = "update tdmisg140" & Comp & " set t_pois = convert(datetime,'" & trdt & "',103) where t_docn='" & t_docn & "' and t_pois < convert(datetime,'31/12/2000',103)"
          Con.Open()
          Cmd.ExecuteNonQuery()
        End Using
      End Using
